...
void TestCuTestInit(CuTest *tc)
{
  ...
  CuAssertTrue(tc, tc2.ran == 0);
  CuAssertTrue(tc, tc2.jumpBuf == NULL);
  if(NULL!=tc2.name) free(tc2.name);
}
...
void TestCuAssert(CuTest* tc)
{
  ...
  CuAssertTrue(tc, tc2->failed);
  CompareAsserts(tc, "CuAssert didn't fail", "test 2", tc2->message);
  if(NULL!=tc2->message) free(tc2->message);
...
void TestCuSuiteRun(CuTest* tc)
{
  ...
  if(NULL!=tc1.name) free(tc1.name);
  if(NULL!=tc2.name) free(tc2.name);
  if(NULL!=tc3.name) free(tc3.name);
  if(NULL!=tc4.name) free(tc4.name);

  if(NULL!=tc1.message) free(tc1.message);
  if(NULL!=tc2.message) free(tc2.message);
  if(NULL!=tc3.message) free(tc3.message);
  if(NULL!=tc4.message) free(tc4.message);
}
...
void TestCuSuiteDetails_SingleFail(CuTest* tc)
{
  ...
  if(NULL!=details.buffer) free(details.buffer);
  if(NULL!=tc1.name) free(tc1.name);
  if(NULL!=tc2.name) free(tc2.name);
...
void TestCuStrCopy(CuTest* tc)
{
  ...
  const char* newStr = catch = CuStrCopy(old);
